...
function createBookDiv(bookObject) {
  ...
  statusButton.addEventListener("click", (event) => {
    event.target.src = toggleBookStatus(
...
function deleteBook(bookDiv) {
  ...
  books.forEach((book) => {
    if (
      book.getAttribute("data-index") === bookDiv.getAttribute("data-index")
    ) {
...
function getBookIndex(bookDiv) {
  let bookIndex = myLibrary.findIndex(
    (bookObject) => bookObject.title === bookDiv.firstChild.textContent
  );
  return bookIndex;
}
...
function putBookOnShelf(newBook) {
  ...
  let bookIndex = myLibrary.findIndex(
    (bookObject) => bookObject.title === newBook.title
  );
